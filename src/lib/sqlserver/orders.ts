...
export async function createOrder(userId: string, items: { slug: string; quantity: number }[], shippingAddress: Record<string, string>, paymentReference?: string) {
  ...
  try {
    ...
    const order = await transaction.request().input("userId", sql.UniqueIdentifier, userId).input("total", sql.Decimal(10, 2), total).input("payment", sql.NVarChar(255), paymentReference || null).input("shipping", sql.NVarChar(sql.MAX), JSON.stringify(shippingAddress || {})).query("INSERT INTO dbo.Orders (user_id, total, payment_reference, shipping_address) OUTPUT INSERTED.id VALUES (@userId, @total, @payment, @shipping)");
