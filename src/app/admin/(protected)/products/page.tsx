...
type AdminProduct = { id: string; name: Record<string, string>; brand: string; category: string; color: string; price: number; stock: number; image_url: string | null; is_active: boolean };
type FormState = { name: string; nameAz: string; nameRu: string; description: string; brand: string; category: string; color: string; price: string; discountPrice: string; stock: string; imageUrl: string; isFeatured: boolean };
const emptyForm: FormState = { name: "", nameAz: "", nameRu: "", description: "", brand: "Euphoria", category: "women", color: "Black", price: "", discountPrice: "", stock: "0", imageUrl: "", isFeatured: false };
...
export default function AdminProductsPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const payload = { name: form.name, description: form.description, nameTranslations: { en: form.name, az: form.nameAz || form.name, ru: form.nameRu || form.name }, descriptionTranslations: { en: form.description, az: form.description, ru: form.description }, brand: form.brand, category: form.category, color: form.color, price: form.price, discountPrice: form.discountPrice, stock: form.stock, imageUrl, isFeatured: form.isFeatured };
  ...
  function edit(product: AdminProduct) {
    setEditingId(product.id); setForm({ ...emptyForm, name: product.name?.en || "", nameAz: product.name?.az || "", nameRu: product.name?.ru || "", brand: product.brand, category: product.category, color: product.color, price: String(product.price), stock: String(product.stock), imageUrl: product.image_url || "" }); window.scrollTo({ top: 0, behavior: "smooth" });
  }
