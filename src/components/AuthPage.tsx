...
const copy: Record<AuthMode, { title: string; text: string; button: string }> = {
  login: { title: "auth.login", text: "auth.loginText", button: "auth.signIn" },
  signup: { title: "auth.signup", text: "auth.signupText", button: "auth.signUp" },
...
export default function AuthPage({ mode }: { mode: AuthMode }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (mode === "login") {
      ...
      if (result) setError(result); else router.push("/account");
      setLoading(false);
      return;
    }
    if (mode === "signup") {
      const result = await register({ firstName: String(form.get("firstName") ?? ""), lastName: String(form.get("lastName") ?? ""), email, phone: String(form.get("phone") ?? "") }, password);
      if (result) setError(result); else { setSubmitted(true); router.push("/account"); }
      setLoading(false);
      return;
    }
    setError("Password reset email delivery must be configured before this feature can be used.");
    setLoading(false);
  };
    ...
    <main className="min-h-[calc(100vh-88px)] lg:min-h-[calc(100vh-108px)]">
      <div className="grid min-h-[700px] lg:grid-cols-2">
        ...
        <div className="flex items-center justify-center px-5 py-16 sm:px-10 lg:px-16 xl:px-24"><div className="w-full max-w-[567px]">
           <h1 className="font-core text-[34px] font-semibold text-[#333]">{t(active.title)}</h1><p className="mt-3 font-causten text-base text-[#666]/80">{t(active.text)}</p>
           {submitted && <div className="mt-6 rounded-soft bg-[#eee5ff] px-4 py-3 font-causten text-sm font-medium text-accent">{mode === "signup" ? "Account created. Check your email if confirmation is required." : mode === "forgot" ? "Reset link sent. Check your inbox to continue." : "Your password has been updated successfully."}</div>}
           {error && <div className="mt-6 rounded-soft bg-[#ffe9e9] px-4 py-3 font-causten text-sm font-medium text-[#a51d2d]">{error}</div>}
          <form onSubmit={submit} className="mt-10 grid gap-6">
            {mode === "signup" && <div className="grid gap-6 sm:grid-cols-2"><label className="grid gap-2 font-causten text-base font-medium text-ink"><span>First name</span><input required name="firstName" placeholder="Jhanvi" className="h-14 rounded-soft border border-line px-5 outline-none placeholder:text-muted/70" /></label><label className="grid gap-2 font-causten text-base font-medium text-ink"><span>Last name</span><input required name="lastName" placeholder="Shah" className="h-14 rounded-soft border border-line px-5 outline-none placeholder:text-muted/70" /></label></div>}
             {mode !== "reset" && <label className="grid gap-2 font-causten text-base font-medium text-ink"><span>{t("auth.email")}</span><span className="flex h-14 items-center gap-3 rounded-soft border border-line px-5"><Mail size={19} className="text-muted" /><input required name="email" type="email" placeholder="example@gmail.com" className="min-w-0 flex-1 bg-transparent outline-none placeholder:text-muted/70" /></span></label>}
             {mode !== "forgot" && <label className="grid gap-2 font-causten text-base font-medium text-ink"><span>{mode === "reset" ? "New password" : t("auth.password")}</span><span className="flex h-14 items-center gap-3 rounded-soft border border-line px-5"><LockKeyhole size={19} className="text-muted" /><input required minLength={8} name="password" type={showPassword ? "text" : "password"} placeholder="Enter your password" className="min-w-0 flex-1 bg-transparent outline-none placeholder:text-muted/70" /><button type="button" onClick={() => setShowPassword(!showPassword)} className="text-muted" aria-label={showPassword ? "Hide password" : "Show password"}>{showPassword ? <EyeOff size={19} /> : <Eye size={19} />}</button></span></label>}
            ...
            {mode === "login" && <div className="flex justify-end"><Link href="/forgot-password" className="font-causten text-sm font-medium text-ink underline underline-offset-4">Forgot password?</Link></div>}
             <button disabled={loading} className="inline-flex h-14 items-center justify-center gap-3 rounded-soft bg-accent px-6 font-causten text-lg font-semibold text-white transition-colors hover:bg-[#7422e0] disabled:opacity-50">{loading ? "Please wait..." : t(active.button)} <ArrowRight size={19} /></button>
          </form>
