...
function CheckoutForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!response.ok) setError(body.error || "Could not create order.");
      ...
      if (!card) { setError("Card details are required."); setSubmitting(false); return; }
      const payment = await stripe.confirmCardPayment(body.clientSecret, { payment_method: { card, billing_details: { name: `${shippingAddress.firstName} ${shippingAddress.lastName}`, email: user.email } } });
